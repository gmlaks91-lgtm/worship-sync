...
const dayCard = `import { BookOpen, Flame, NotebookPen } from "lucide-react";
  ...
  return (
    <article className="surface-card-hover relative p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <motion className="rounded-full border border-sky-100 bg-sky-50 px-2.5 py-1 text-xs font-semibold text-sky-700">
          {points}P
        </motion>
      </div>
...
fs.writeFileSync(path.join(root, "src/features/dashboard/components/WeeklyChecklistDayCard.tsx"), dayCard, "utf8");
console.log("day card ok", dayCard.includes("\uD55C\uC904"));
