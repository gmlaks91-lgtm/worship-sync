-- =============================================================================
-- Teams & team_members — 경건 일지 다중 소속·팀별 피드
-- =============================================================================
...
create policy "team_members_select_shared"
  ...
  using (
    user_id = auth.uid()
    or public.shares_team_with(auth.uid(), user_id)
  );
...
-- ---------------------------------------------------------------------------
-- weekly_checklists: 같은 팀(들) 소속 팀원 일지 읽기
-- (작성·수정은 기존 own 정책 유지 — 제출 시 모든 소속 팀 피드에 자동 노출)
-- ---------------------------------------------------------------------------
